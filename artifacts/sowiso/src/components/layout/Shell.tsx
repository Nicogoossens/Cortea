...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  const navigation = [
    { name: "The Dashboard", href: "/", icon: Landmark },
    { name: "The Atelier", href: "/atelier", icon: BookOpen },
    { name: "The Counsel", href: "/counsel", icon: Shield },
    { name: "The Compass", href: "/compass", icon: Compass },
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      ...
      <div className="md:hidden flex items-center justify-between p-4 border-b border-border bg-sidebar text-sidebar-foreground">
        <span className="font-serif text-xl tracking-wide text-sidebar-primary">SOWISO</span>
        <Button variant="ghost" size="icon" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-[65px] bg-sidebar z-50 p-4 animate-in fade-in slide-in-from-top-4">
      ...
      <aside className="hidden md:flex w-64 flex-col bg-sidebar text-sidebar-foreground border-r border-sidebar-border sticky top-0 h-screen">
        <div className="p-8 flex items-center justify-center border-b border-sidebar-border/50">
          <span className="font-serif text-3xl tracking-widest text-sidebar-primary uppercase">SOWISO</span>
        </div>
        ...
        <div className="p-6 border-t border-sidebar-border/50 text-xs text-sidebar-foreground/50 text-center font-mono uppercase tracking-widest">
          Est. 2024
        </div>
      </aside>
