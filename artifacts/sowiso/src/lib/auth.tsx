...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const logout = useCallback(() => {
    ...
    let shouldClearServer = true;
    try {
      const privacyKeyForRemember = currentUserId
        ? `sowiso_privacy_settings_${currentUserId}`
        : "sowiso_privacy_settings";
      const rawForRemember = localStorage.getItem(privacyKeyForRemember);
      if (rawForRemember) {
        const parsed = JSON.parse(rawForRemember) as { rememberPreferences?: boolean };
        if (parsed.rememberPreferences === true) {
          shouldClearServer = false;
        }
      }
    } catch {
      // ignore
    ...
    // Clear privacy_settings on the server only when "Remember preferences" is disabled
