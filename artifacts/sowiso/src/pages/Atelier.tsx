  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="space-y-4 max-w-3xl">
        <h1 className="text-4xl md:text-5xl font-serif text-foreground">The Atelier</h1>
        <p className="text-muted-foreground text-lg font-light leading-relaxed">
          Refine your instincts through practice. Select a scenario below to test your judgement in complex social and cultural situations.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {scenarios?.map((scenario) => (
            <Link key={scenario.id} href={`/atelier/${scenario.id}`}>
              <Card className="h-full border-border bg-card transition-all duration-300 hover:shadow-md hover:border-primary/40 cursor-pointer flex flex-col">
                ...
                <CardFooter className="pt-4 border-t border-border/50 text-xs text-muted-foreground flex justify-between items-center bg-muted/20">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{scenario.estimated_minutes || 2} min</span>
                  </div>
          ...
          {(!scenarios || scenarios.length === 0) && (
            <div className="col-span-full py-12 text-center text-muted-foreground border border-dashed border-border rounded-sm bg-muted/10">
              ...
              <p className="font-serif text-xl">No scenarios currently available.</p>
              <p className="text-sm mt-2">The curator is preparing new materials.</p>
            </div>
          )}
        </div>
      )}
