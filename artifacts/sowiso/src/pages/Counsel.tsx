        ...
        <Card className="border-border bg-card shadow-sm">
          <CardContent className="p-6 md:p-8 space-y-8">
            <fieldset className="space-y-4">
              <legend className="text-sm font-medium text-foreground tracking-wide">Select Domain</legend>
              <div className="flex flex-wrap gap-2" role="group" aria-label="Select domain">
                {CATEGORIES.map(cat => (
                  <Badge
                    key={cat}
                    ...
                    tabIndex={0}
                    onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") setSelectedCategory(selectedCategory === cat ? null : cat); }}
                  >
                    {cat}
                  </Badge>
                ))}
            ...
            <div className="space-y-4">
              <label htmlFor="counsel-query" className="text-sm font-medium text-foreground tracking-wide block">
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    <span aria-live="assertive">Consulting…</span>
                  </>
                ) : (
                  <>
                    {t("counsel.request")}
        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-4" aria-live="polite" aria-atomic="true">
          <Card className="border-primary/20 bg-card shadow-md relative overflow-hidden">
            ...
            <CardHeader className="pb-2 pt-8 px-8">
              <CardDescription className="uppercase tracking-widest text-xs font-semibold text-primary">{t("counsel.guidance")}</CardDescription>
            </CardHeader>
