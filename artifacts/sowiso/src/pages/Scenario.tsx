...
export default function Scenario() {
  ...
  if (!scenario) {
    return (
      <div className="text-center py-20">
        ...
        <h2 className="font-serif text-2xl">Scenario Not Found</h2>
  ...
  const handleSelect = (index: number) => {
    if (result) return; // Prevent selection after answer
    ...
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 text-sm text-primary font-mono tracking-widest uppercase">
          <span>Pillar {scenario.pillar}</span>
      ...
      {!result && (
        <div className="pt-6 flex justify-end">
          ...
          >
            {submitAnswer.isPending ? "Submitting..." : "Confirm Decision"}
          </Button>
        </div>
      )}

      {result && (
        <Card className="mt-8 border-primary/20 bg-primary/5 shadow-md animate-in slide-in-from-bottom-4">
          <CardHeader>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between py-4 border-t border-border/50 gap-4">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground uppercase tracking-widest font-mono">Impact</span>
                <div className={`text-xl font-medium ${result.score_delta > 0 ? 'text-green-600' : result.score_delta < 0 ? 'text-red-600' : 'text-muted-foreground'}`}>
                  {result.score_delta > 0 ? '+' : ''}{result.score_delta} Noble Score
                </div>
              </div>
              
              {result.level_up && (
                <div className="bg-secondary/20 text-secondary-foreground px-4 py-2 rounded-sm text-center border border-secondary/30">
                  <div className="text-xs uppercase tracking-widest mb-1 opacity-80">Promotion</div>
                  <div className="font-serif font-medium">Elevated to {result.new_level_name}</div>
                </div>
              )}
            </div>
          </CardContent>
