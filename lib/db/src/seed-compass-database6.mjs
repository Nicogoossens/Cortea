#!/usr/bin/env node
/**
 * Parses attached_assets/Compas_database6_*.md and:
 *   1. Upserts each country into compass_regions (en-GB locale, is_published=true)
 *   2. Writes the venues to artifacts/api-server/src/data/venues-extra5.ts
 ...
 *      lib/db/.tmp-mehrabian-extra5.txt for splicing into culture.ts
 *
 * Run with:
 *   node lib/db/src/seed-compass-database6.mjs
 */
...
const MD_PATH = resolve(ROOT, "attached_assets/Compas_database6_1777810672073.md");
const VENUES_OUT = resolve(ROOT, "artifacts/api-server/src/data/venues-extra5.ts");
const MEHRABIAN_OUT = resolve(ROOT, "lib/db/.tmp-mehrabian-extra5.txt");
...
function writeVenuesFile(allVenues) {
  const header = `// AUTO-GENERATED by lib/db/src/seed-compass-database6.mjs — do not edit by hand.
// Source: attached_assets/Compas_database6_1777810672073.md
import type { Venue } from "./venues.js";

export const EXTRA_VENUES5: Venue[] = [
`;
...
async function main() {
  ...
  const unmatched = sections.filter((s) => !COUNTRY_CODES[s.name]).map((s) => s.name);
  if (unmatched.length) {
